...
class AttackLoader():
    def __init__(self, args, logger, data_type="standard", target_csv='log'):
        self.cache_dir = "./attack-log/cache"
        self.logger = logger
        self.scenario = args.scenario
        self.max_adv_num_dict = {'imdb': 1000, 'ag-news': 2000, 'sst2': 1000, 'snli': 2000, 'paws': 2000, 'mnli': 1000,
                                 'hate': 1000, 'yelp': 1000}
        ...
        if data_type == "standard":
            ...
            self.model_dir = self.data_dir + '/' + args.model_type
            self.csv_dir = self.model_dir + '/' + args.attack_type
            print('csv_dir', self.csv_dir)
            csv_file = self.csv_dir + '/' + target_csv + ".csv"
    ...
    def get_attack_from_csv(self, dtype='test', batch_size=64,
        ...
        if self.scenario == 's1':
            ...
            target_samples = max_adv_num * (1 / adv_sr) * (
                    1 / task_acc)  # Expected number of required sample to attain max_adv_num adversarial samples
