...
def get_test_features(model_wrapper, batch_size, dataset, text_key, params, logger=None, feat_type='cls',
                      return_probs=False):
    ...
    for idx, text in tqdm(enumerate(texts[:])):
        ...
        for prob in prob_list:
            for _ in range(samples_per_prob):
                masked_text = [t if (random.uniform(0, 1) > prob or t in ignore_words or idx <= indice) else '[MASK]'
