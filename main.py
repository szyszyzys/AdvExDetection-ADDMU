...
parser.add_argument("--include_fae", default=False, action='store_true',
...
if __name__ == "__main__":
    ...
    train_ratio_dict = {"sst2": 0.5, "ag-news": 0.1, "yelp": 0.1, "imdb": 0.1, "snli": 0.1, "mnli": 0.1, "paws": 0.5,
                        "liar": 0.5, "hate": 1.0}
    ...
    s_time = time.time()
    feat_type = 'cls'
    feats, feats_by_label = get_train_features(model_wrapper, args, batch_size=16, dataset=trainset, text_key=text_key,
                                               feat_type=feat_type,
    ...
    if args.detect_method == 'rde':
        ...
        train_stats, estimators = get_stats(reduced_feat, labels, cov_estim_name=args.cov_estimator,
                                            use_shared_cov=params['shared_cov'], params=params)
        naive_train_stats, naive_estimators = get_stats(reduced_feat, labels, cov_estim_name="None",
    ...
    for s in range(args.start_seed, args.end_seed + 1):
        logger.set_seed(s)
        # load attack data csv
        loader = AttackLoader(args, logger, data_type=args.data_type)
        detector = Detector(model_wrapper, all_train_stats, loader, logger, params, (
            scaler, reducer, all_estimators, args.cov_estimator) if args.detect_method == 'rde' else None,
                            use_val=args.use_val, dataset=args.dataset, seed=s)
        if args.detect_method == 'ppl':
            detector.test_baseline_PPL(args.fpr_threshold)
        elif args.detect_method == 'ue':
            from sklearn.model_selection import train_test_split

            feats, feats_by_label = get_train_features(model_wrapper, args, batch_size=16, dataset=trainset,
                                                       feat_type=feat_type,
            ...
            if isinstance(text_key, tuple):
                ...
                p_train, p_test, h_train, h_test, y_train, y_test = train_test_split(premise, hypothesis,
                                                                                     trainset['label'], test_size=0.95,
            ...
            training_probs, training_uncertainty = detector.get_train_stats(X_train, model_name=args.target_model,
                                                                            dataset_name=args.dataset,
                                                                            text_key=text_key)

            detector.get_ue(args.fpr_threshold, feats_by_label, training_probs, training_uncertainty, text_key=text_key,
                            feat_type=feat_type, mu_aug=args.mu_aug, du_aug=args.du_aug, mu_iters=args.mu_iters)
        elif args.detect_method == 'rde':
            roc, auc, tpr_at_fpr, conf, testset = detector.test(args.fpr_threshold, args.pkl_test_path,
                                                                feat_type=feat_type, text_key=text_key)
