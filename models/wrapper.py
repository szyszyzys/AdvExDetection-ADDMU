...
class BertWrapper:
    def __init__(self, config, logger):
        self.config = config
        max_seq_len_dict = {"imdb": 512, "sst2": 64, "ag-news": 256, "snli": 64, "paws": 64, 'mnli': 64, 'hate': 64,
                            'yelp': 128}
    ...
    def inference(self, text, output_hs=False, output_attention=False):
        ...
        if isinstance(text, tuple):
            ...
            x = self.tokenizer(*input, max_length=self.max_seq_len, add_special_tokens=True, padding="max_length",
                               truncation=True,
                               return_attention_mask=True, return_tensors='pt')
            masks = self.get_second_sentence_mask(x)
        else:
            text = self.__pre_process(text)
            x = self.tokenizer(text, max_length=self.max_seq_len, add_special_tokens=True, padding="max_length",
                               truncation=True,
                               return_attention_mask=True, return_tensors='pt')
        self.seq_len_cache.extend(x.attention_mask.sum(-1).tolist())

        output = self.model(input_ids=x['input_ids'].to(self.device),
